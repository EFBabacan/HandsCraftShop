...
namespace HandCraft.Siparis.Application.Features.Queries.GetSiparislerByUserId.Handlers
{
    public class GetSiparislerByUserIdQueryHandler
        : IRequestHandler<GetSiparislerByUserIdQuery, List<SiparisListDto>>
    {
        private readonly SiparisDbContext _context;

        public GetSiparislerByUserIdQueryHandler(SiparisDbContext context)
        {
            _context = context;
        }

        public async Task<List<SiparisListDto>> Handle(
            GetSiparislerByUserIdQuery request, CancellationToken cancellationToken)
        {
